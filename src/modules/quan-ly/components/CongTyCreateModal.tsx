...
const CongTyCreateModal: React.FC<CongTyCreateModalProps> = ({ onClose, onSuccess }) => {
  const [formData, setFormData] = useState<CreateCongTyRequest>({
    ma_cong_ty: '',
    ten_cong_ty: '',
    dia_chi: '',
    so_dien_thoai: '',
  ...
  const validateForm = async (): Promise<boolean> => {
    ...
    if (formData.ma_cong_ty.trim()) {
      try {
        const exists = await congTyService.checkMaCongTyExists(formData.ma_cong_ty.trim());
        if (exists) {
  ...
  const handleInputChange = (field: keyof CreateCongTyRequest, value: string) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await congTyService.createCongTy({
        ...formData,
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Nhập thông tin công ty cần thêm vào hệ thống
              </p>
            </div>
          </div>
          <button
...
export default CongTyCreateModal;
